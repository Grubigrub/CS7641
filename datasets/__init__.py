...
class DigitsData():
    ...
    def populate(self):
            ...
            seed(2)
            shuffle(self.data)
            self.training = self.data[:9 * n // 10]
            self.testing = self.data[9 * n // 10:]
...
class WaveformData:
    ...
    def populate(self):
            n = len(self.data)
            shuffle(self.data)
            self.training = self.data[n // 10:]
            self.testing = self.data[:n // 10]
...
with open('datasets/waveform.data', encoding='utf8') as csvFile:
